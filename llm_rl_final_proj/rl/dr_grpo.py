        ...
        for _ in range(cfg.ppo_epochs):
            for mb in iter_minibatches(
                ...
                new_logp = compute_per_token_logprobs(
                    model, mb.input_ids, mb.attention_mask, enable_grad=True
                )
                ...
                clipped_pos = ((ratio < (1.0 - cfg.clip_eps)) | (ratio > (1.0 + cfg.clip_eps))).float()
                clipfrac = masked_mean(clipped_pos, mask)
